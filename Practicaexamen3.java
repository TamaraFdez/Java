...
public class Practicaexamen3 {
    public static void main(String[] args) {
        ...
        System.out.println(
                localizarMatricula("5555-LLL", new String[] { "1112-AAA", "4444-LLL", "5555-LLL", "9999-ZZZ" }));
        System.out.println(Arrays.toString(resumenAlquileres(new int[] { 0, 120, 0, 90 },
                new String[] { "1111-AAA", "2222-BBB", "3333-CCC", "4444-DDD" })));
    }
    ...
    public static int[][] gatos(String[] mapa) {
        int[][] coords = new int[3][2];
        int encontrados = 0;
        for (int i = 0; i < mapa.length; i++) {
            for (int j = 0; j < mapa.length; j++) {
                char c = mapa[i].charAt(j);
                if (esLetra(c)) {

                    if (c == 'L' || c == 'M' || c == 'R') {
                        coords[encontrados][0] = i;
                        coords[encontrados][1] = j;
                        encontrados++;
                    }
                }
            }
        }
        int[][] result = new int[encontrados][2];
    ...
    public static double distancia(int[] a, int[] b) {
        int dx = a[0] - b[0];
        int dy = a[1] - b[1];
        return Math.sqrt(dx * dx + dy * dy);
    }

    public static boolean gatosSeparados(String[] mapa, double minDist) {
        int[][] gatos = gatos(mapa);

        for (int i = 0; i < gatos.length; i++) {
            for (int j = i + 1; j < gatos.length; j++) {
                if (distancia(gatos[i], gatos[j]) < minDist) {
                    return false;
                }
            }
        }
